...
const navItems = [
  ...
  },
  {
    ...
    icon: Store01Icon,
    label: "Inventory",
    roles: ["ADMIN", "MANAGER", "WAREHOUSE_OPERATOR", "QC_INSPECTOR"],
  },
  {
    href: "/suppliers",
    icon: Building04Icon,
  ...
  },
  {
    ...
    roles: ["ADMIN", "MANAGER", "QC_INSPECTOR"],
  },
  {
    href: "/movements",
...
const adminItems = [{ href: "/users", icon: UserMultipleIcon, label: "Users" }];
  ...
  return (
    <aside className="fixed left-0 top-0 h-full w-64 bg-white border-r border-gray-100 flex flex-col z-40 shrink-0">
      ...
      <div className="flex items-center gap-3 px-6 py-6">
        ...
        <div>
          <p className="text-brand-black font-bold text-base leading-tight">
            Abibas
          </p>
          <p className="text-brand-muted text-[11px] font-medium tracking-wide">
            WMS
          </p>
        </div>
      </div>
        ...
        {navItems
          ...
          .map(({ href, icon: Icon, label }) => (
            <Link
              ...
              <Icon
                size={20}
                primaryColor={isActive(href) ? "#090909" : "#788596"}
              />
        ...
        <DropdownMenu>
          <DropdownMenuTrigger className="group flex items-center gap-3 rounded-xl px-2 py-2 transition-colors hover:bg-brand-bg">
            ...
            <div className="min-w-0 flex-1">
              <p className="text-brand-black text-sm font-semibold truncate">
                {user?.name}
              </p>
